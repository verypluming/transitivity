...
lex(tv,[surf:[know],symbol:[know],infl:base]).
lex(tv,[surf:[knew],symbol:[know],infl:past]).
...
lex(tv,[surf:[remember],symbol:[remember],infl:base]).
lex(tv,[surf:[remembered],symbol:[remember],infl:past]).
...
semparse(D,S) :-
   ...
   random_member(Atom,Atoms),
   % attachNot(Atoms,NegAtoms),
   % append(Atoms,NegAtoms,Sub),
   Sub = [Atom,not(Atom)],
   write(Sub),
   write('\t'),
   length(Sub,L),
